...
class DataProcessor {
  ...
  protected void calc(MovementData data) {
    Float accCurrent = (float) Math
        .sqrt(data.accX * data.accX + data.accY * data.accY + data.accZ * data.accZ);
    Float gyroCurrent = (float) Math
        .sqrt(data.posX * data.posX + data.posY * data.posY + data.posZ * data.posZ);

    //compare new value to maxima/minima
    if (accCurrent > accMax) {
      accMax = accCurrent;
    }
    if (gyroCurrent > gyroMax) {
      gyroMax = gyroCurrent;
    }
    if (accMin < 0 || accMin < accCurrent) {
      accMin = accCurrent;
    }
    if (gyroMin < 0 || gyroMin < gyroCurrent) {
      gyroMin = gyroCurrent;
    }
    if (accCurrent > accMaxLongTerm) {
      accMaxLongTerm = accCurrent;
    }
    if (gyroCurrent > gyroMaxLongTerm) {
      gyroMaxLongTerm = gyroCurrent;
    }
      ...
      if (accLongTerm.size() == 251) {
        ...
        if (accMaxLongTerm.equals(accLastLongTerm)) {
          ...
          for (int i = 0; i < accLongTerm.size(); i++) {
            if (accMaxLongTerm < accLongTerm.get(i)) {
              accMaxLongTerm = accLongTerm.get(i);
            }
          }
        }
        if (gyroMaxLongTerm.equals(gyroLastLongTerm)) {
          ...
          for (int i = 0; i < gyroLongTerm.size(); i++) {
            if (gyroMaxLongTerm < gyroLongTerm.get(i)) {
              gyroMaxLongTerm = gyroLongTerm.get(i);
            }
          }
        }
